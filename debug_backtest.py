...
for i in range(start_idx, start_idx + 10):
    ...
    print(f"\n--- {date.date()} ---")
    print(f"Eligible universe size: {len(valid)}")
    print(f"Number selected (Top 20%): {n_select}")
    
    if n_select > 0:
        ret_row = monthly_ret.iloc[i]
        portfolio_returns = [ret_row.get(t, 0.0) for t in new_selected]
        # Cap returns logic as implemented
        portfolio_returns = [min(max(r, -0.90), 1.0) if not pd.isna(r) else 0.0 for r in portfolio_returns]
        print(f"Average return of selected portfolio this month: {np.mean(portfolio_returns)*100:.2f}%")
        
        # Who are the worst offenders?
        worst = sorted(list(zip(new_selected, portfolio_returns)), key=lambda x: x[1])[:5]
        print(f"Worst stocks in portfolio: {worst}")
