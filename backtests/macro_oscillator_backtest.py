"""
B3 Macro Oscillator (Global Flight-to-Quality)
=======================================================================
Strategy: Brazil is a highly cyclical, emerging market. When the BRL 
crashes, capital flees to the US Dollar. This strategy uses a macro 
trend-following filter on the Ibovespa index to dictate capital allocation:

- Risk ON (IBOV > 10M SMA): Deploy 100% of capital into the B3 Multi-Factor 
  Stock Portfolio (High Momentum + Low Volatility).
- Risk OFF (IBOV < 10M SMA): Deploy 100% of capital into IVVB11 
  (The S&P 500 ETF in BRL) to capture the USD/BRL currency devaluation.
"""
import warnings
...
def generate_signals(adj_close, close_px, fin_vol, ibov_px):
    ...
    for i in range(start_idx, len(ret)):
        ...
        if not bull_regime:
            # Risk OFF: 100% US Dollar / S&P 500
            target_weights.iloc[i, target_weights.columns.get_loc("IVVB11_ASSET")] = 1.0
            prev_sel = set()
            continue
            
        # Risk ON: 100% B3 Multi-Factor Equities
...
def main():
    print("\n" + "=" * 70)
    print("  B3 MACRO OSCILLATOR: B3 MULTI-FACTOR <-> IVVB11 (15% CGT)")
    print("=" * 70)
    ...
    plot_tax_backtest(
        ...
        out_path="macro_oscillator_backtest.png",
