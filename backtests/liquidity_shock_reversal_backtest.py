"""
B3 Liquidity Shock Reversal Backtest
=======================================================================
Academia: Campbell, Grossman, and Wang (1993)
A stock that drops 15% on high volume is fundamentally dying.
A stock that drops 15% on low volume is suffering a transient liquidity shock.
We buy the stocks that dropped the most, but only if their volume ALSO dropped.
"""
...
REBALANCE_FREQ = "ME"
LOOKBACK_YEARS = 0.5  # 6 month avg volume
...
def generate_signals(adj_close, close_px, fin_vol):
    ...
    # We need total period volume for the shock ratio, not just daily average
    period_vol = fin_vol.resample(REBALANCE_FREQ).sum()
    adtv = fin_vol.resample(REBALANCE_FREQ).mean()

    # ── Signal 1: 1-Month Reversal ──
    # Rank by lowest return (most negative)
    rev_rank = (-ret).rank(axis=1, pct=True)

    # ── Signal 2: Volume Shock ──
    # Current volume / 6-month moving average volume
    avg_vol = period_vol.shift(1).rolling(LOOKBACK_PERIODS).mean()
    vol_shock = period_vol / avg_vol

    # Rank by lowest volume shock (quietest drop)
    vol_rank = (-vol_shock).rank(axis=1, pct=True)

    # Composite: Best reversal + Best low-volume shock
    signal = (rev_rank * 0.5) + (vol_rank * 0.5)
...
def main():
    print("\n" + "=" * 70)
    print("  B3 LIQUIDITY SHOCK REVERSAL BACKTEST")
    print("=" * 70)
    ...
    plot_tax_backtest(
        title=f"Liquidity Shock Reversal (Price + Volume Drop)\nTop {int(TOP_DECILE * 100)}%  ·  R$ {MIN_ADTV / 1_000_000:.0f}M+ ADTV  ·  15% CGT\n{START_DATE[:4]}–{END_DATE[:4]}",
        ...
        out_path="liquidity_shock_backtest.png",
