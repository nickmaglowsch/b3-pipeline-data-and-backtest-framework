"""
B3 Bull Trap Short Strategy
=======================================================================
Strategy: We proved that a massive 1-week spike in the Brazilian market
is the ultimate retail "Bull Trap" and highly prone to mean reversion.

This strategy intentionally hunts for those exact 1-week blow-off tops,
but instead of buying them, it SHORTS them. It allocates 100% of capital
to a risk-free CDI margin account, and uses it as collateral to sell short
the Top 10% of stocks with the highest 1-week acceleration.
"""
...
RECENT_MOM_PERIODS = 1  # 1 week
HISTORICAL_MOM_PERIODS = 4  # 4 weeks (1 month)

# We want to react instantly to the trap, so no skipped periods
SKIP_PERIODS = 0

TOP_DECILE = 0.10
TAX_RATE = 0.15

# Short selling is expensive. We assume a 0.2% slippage + borrow fee
SLIPPAGE = 0.002
...
# High ADTV is strictly required for shorting to ensure borrow inventory exists
MIN_ADTV = 5_000_000
...
def generate_signals(adj_close, close_px, fin_vol):
    ...
    historical_mom = (
        log_ret.shift(SKIP_PERIODS + RECENT_MOM_PERIODS)
        .rolling(HISTORICAL_MOM_PERIODS)
        .sum()
    )
    ...
    has_glitch = (
        ((ret > 1.0) | (ret < -0.90))
        .shift(SKIP_PERIODS)
        .rolling(RECENT_MOM_PERIODS + HISTORICAL_MOM_PERIODS)
        .max()
    )
    mom_of_mom[has_glitch == 1] = np.nan

    target_weights = pd.DataFrame(0.0, index=ret.index, columns=ret.columns)
    # We will hold our cash in CDI as collateral (100%)
    target_weights["CDI_ASSET"] = 1.0
    ...
    for i in range(start_idx, len(ret)):
        ...
        # We only want to short highly liquid stocks
        valid_mask = (adtv_row >= MIN_ADTV) & (raw_close_row >= 5.0)
        ...
        if len(valid) < 5:
            ...
            # Get the ones with the HIGHEST acceleration (the blow off tops)
            sel = set(valid.nlargest(n_sel).index)

        if not sel:
            continue

        # We short them safely!
        # By only using -0.20 exposure to the short targets, we leave 100% in CDI.
        # This prevents a single 300% short-squeeze from bankrupting the whole portfolio!
        weight_per_stock = -0.20 / len(sel)
        for t in sel:
...
def main():
    print("\n" + "=" * 70)
    print(f"  B3 BULL TRAP (SHORT 1W vs 4W ACCELERATION) BACKTEST")
    print("=" * 70)
    ...
    plot_tax_backtest(
        title=f"Short The Bull Trap  ·  Shorting Top {int(TOP_DECILE * 100)}% 1W vs 4W Spikes\n100% CDI Collateral  ·  15% CGT + {SLIPPAGE * 100}% Slippage/Borrow Fee\n{START_DATE[:4]}–{END_DATE[:4]}",
        ...
        out_path="bull_trap_short_backtest.png",
