"""
B3 Momentum of Momentum (Acceleration) - Contrarian Backtest
This backtest captures the anomaly by doing the opposite selection:
instead of going long the top accelerating names, it goes long the
most negative accelerations (contrarian to the original signal).
"""
...
def generate_signals(adj_close, close_px, fin_vol):
    ...
    for i in range(start_idx, len(ret)):
        ...
        if len(valid) < 5:
            ...
            # Instead of picking the top accelerators, pick the bottom (most negative)
            n_sel = max(1, int(len(valid) * TOP_DECILE))
            sel = set(valid.nsmallest(n_sel).index)
...
def main():
    print("\n" + "=" * 70)
    print(
        f"  B3 {RECENT_MOM_PERIODS}M vs {HISTORICAL_MOM_PERIODS}M ACCELERATION CONTRARIAN BACKTEST"
    )
    print("=" * 70)
    ...
    print("\n🧠 Generating target weights matrix (contrarian)...")
    ...
    plot_tax_backtest(
        title=f"Momentum Acceleration (Contrarian)  ·  {RECENT_MOM_PERIODS}M vs {HISTORICAL_MOM_PERIODS}M (Skip 1)\nBottom {int(TOP_DECILE * 100)}%  ·  R$ {MIN_ADTV / 1_000_000:.0f}M+ ADTV  ·  15% CGT + {SLIPPAGE * 100}% Slippage\n{START_DATE[:4]}–{END_DATE[:4]}",
        ...
        out_path="mom_of_mom_inverse_backtest.png",
