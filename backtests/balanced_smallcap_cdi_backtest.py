"""
Balanced Small-Cap Momentum + CDI 50/50 Backtest
=================================================
Allocates 50% to the small-cap momentum strategy and 50% to CDI,
rebalanced monthly.  Models a balanced portfolio capturing the small-cap
momentum premium while dampening volatility with Brazilian fixed income.

Signal generation uses the same logic as smallcap_momentum_backtest.py:
6M lookback, 1M skip, bottom 50% ADTV, top 20% momentum, R$1M floor.
Stock weights are scaled by 0.5 and CDI_ASSET gets 0.5 each period.
...
REBALANCE_FREQ = "ME"
LOOKBACK_PERIODS = 6  # 6-month momentum
...
def generate_signals(adj_close, close_px, fin_vol, cdi_ret):
    """
    Build equal-weight target weights for 50/50 small-cap momentum + CDI.

    1. Resample to monthly, compute ADTV and log returns
    2. Liquidity filter: ADTV >= R$1M and price >= R$1.0
    3. Small-cap filter: bottom 50% by ADTV (median of liquid universe)
    4. Momentum signal: 6-month cumulative log return with 1-month skip
    5. Selection: top 20% by momentum from small-cap subset
    6. Equal weight across selected stocks, scaled by 0.5
    7. CDI_ASSET gets 0.5 weight each active period
    """
    ...
    signal = log_ret.shift(SKIP_PERIODS).rolling(LOOKBACK_PERIODS).sum()
    ...
    for i in range(start_idx, len(ret)):
        ...
        if len(liquid_universe) < 5:
            ...
            # Small-cap filter: bottom 50% by ADTV within liquid universe
            liquid_adtv = adtv_row[liquid_universe.index]
            adtv_median = liquid_adtv.median()
            smallcap_mask = liquid_adtv <= adtv_median
            smallcap_universe = liquid_universe[smallcap_mask]

            if len(smallcap_universe) < 3:
                sel = prev_sel
            else:
                # Top 20% by momentum from small-cap subset
                n_sel = max(1, int(len(smallcap_universe) * TOP_QUINTILE))
                sel = set(smallcap_universe.nlargest(n_sel).index)

        if not sel:
            continue

        # Scale stock weights by WEIGHT_EQUITY (0.5)
        weight_per_stock = WEIGHT_EQUITY / len(sel)
        for t in sel:
            target_weights.iloc[i, target_weights.columns.get_loc(t)] = weight_per_stock

        # CDI gets WEIGHT_CDI (0.5)
...
def plot_balanced_smallcap_cdi(
    title,
    pretax_val,
    aftertax_val,
    deferred_val,
    ...
    total_tax_std,
    total_tax_deferred,
    out_path="balanced_smallcap_cdi_backtest.png",
):
...
def main():
    print("\n" + "=" * 70)
    print("  BALANCED SMALL-CAP MOMENTUM + CDI (50/50) BACKTEST")
    ...
    print("\n  Generating balanced small-cap momentum + CDI signals...")
    ...
    result_deferred = run_simulation(
        ...
        tax_rate=TAX_RATE,
        slippage=SLIPPAGE,
        name="Balanced SmallCap+CDI (Deferred DARF)",
        monthly_sales_exemption=20_000,
        defer_tax=True,
    )
    ...
    # ── Plot ──────────────────────────────────────────────────────
    plot_balanced_smallcap_cdi(
        title=(
            f"Balanced Small-Cap Momentum + CDI (50/50)  ·  "
            f"{LOOKBACK_PERIODS}M Lookback · Skip {SKIP_PERIODS}\n"
            f"Bottom 50% ADTV · Top {int(TOP_QUINTILE * 100)}% Momentum · "
