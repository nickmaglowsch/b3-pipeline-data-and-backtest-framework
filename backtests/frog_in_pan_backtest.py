"""
B3 Frog-in-the-Pan (Continuous Information) Backtest
=======================================================================
Academia: Da, Gurun, and Warachka (2014)
Investors under-react to small, continuous positive news. We rank stocks by
their "Win Rate" (number of positive months in the lookback) and combine
it with their actual Momentum.
"""
...
def generate_signals(adj_close, close_px, fin_vol):
    ...
    # ── Frog in the Pan (Win Rate) ──
    is_positive = (ret > 0).astype(float)
    # Give it some nan handling so it doesn't count 0 for missing data
    is_positive[ret.isna()] = np.nan
    win_rate = is_positive.shift(SKIP_PERIODS).rolling(LOOKBACK_PERIODS).sum()
    win_rank = win_rate.rank(axis=1, pct=True)

    # Composite
    signal = (mom_rank * 0.5) + (win_rank * 0.5)
...
def main():
    print("\n" + "=" * 70)
    print("  B3 FROG-IN-THE-PAN (WIN RATE MOMENTUM) BACKTEST")
    print("=" * 70)
    ...
    plot_tax_backtest(
        ...
        out_path="frog_in_pan_backtest.png",
