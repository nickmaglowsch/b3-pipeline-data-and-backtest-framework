"""
B3 Multi-Factor Strategy Backtest (Top 20, Sharpe-Weighted)
=======================================================================
Strategy: Instead of picking a percentage of the universe and equal-weighting,
this specifically picks exactly the Top 20 ranking stocks every month,
and dynamically weights them according to their historical Sharpe Ratio 
(stocks with a higher Sharpe get more capital).
"""
import warnings
...
PORTFOLIO_SIZE = 20  # Always buy exactly 20 stocks
TAX_RATE = 0.15
SLIPPAGE = 0.001
...
def generate_signals(adj_close, close_px, fin_vol):
    ...
    vol_signal = -ret.shift(SKIP_PERIODS).rolling(LOOKBACK_PERIODS).std()
    vol_glitch = ((ret > 1.0) | (ret < -0.90)).shift(SKIP_PERIODS).rolling(LOOKBACK_PERIODS).max()
    ...
    # Since we need to weight by Sharpe, we need actual positive sharpe values
    # We will use Return / Volatility
    # Add a small epsilon to volatility to prevent division by zero
    vol = ret.shift(SKIP_PERIODS).rolling(LOOKBACK_PERIODS).std() + 1e-6
    sharpe = mom_signal / vol
    
    # Floor Sharpe at 0 (so we don't assign negative weights).
    # If a stock is in the top 20 composite but somehow has a negative return over the last year,
    # it gets 0 weight. Alternatively, we can clip to a small positive number.
    ...
    for i in range(start_idx, len(ret)):
        ...
        # Select the absolute Top N
        winners = valid.nlargest(PORTFOLIO_SIZE).index
        
        # Extract their Sharpe ratios
...
def main():
    print("\n" + "=" * 70)
    print("  B3 NATIVE MULTI-FACTOR (TOP 20, SHARPE WEIGHTED)")
    print("=" * 70)
    ...
    plot_tax_backtest(
        ...
        out_path="multifactor_top20_sharpe.png",
        cdi_ret=cdi_ret
    )
