"""
B3 Small-Cap + Momentum Overlay Backtest
=========================================
Exploits the small-cap premium and momentum bursts driven by retail flows,
lower analyst coverage, and speculative cycles in the Brazilian market.

Filters for small-cap stocks (bottom 50% by ADTV as market-cap proxy),
selects top 20% by 6-month momentum (skip 1), and compares three tax
scenarios: no tax, standard 15% CGT, and 15% CGT with deferred DARF
payment (tax due on last business day of T+1).
"""
...
def generate_signals(adj_close, close_px, fin_vol):
    """
    Build equal-weight target weights for small-cap momentum strategy.
...
def plot_smallcap_momentum(
    ...
    total_tax_std,
    total_tax_deferred,
    out_path="smallcap_momentum_backtest.png",
):
...
def main():
    print("\n" + "=" * 70)
    print("  B3 SMALL-CAP + MOMENTUM OVERLAY BACKTEST")
    ...
    print("\n  Generating small-cap momentum signals...")
    ...
    # ── Plot ──────────────────────────────────────────────────────
    plot_smallcap_momentum(
        title=(
            f"Small-Cap + Momentum Overlay (B3)  ·  "
